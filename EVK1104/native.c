...
char clearDisplay(){
  et024006_DrawFilledRect(0, 0, ET024006_WIDTH, ET024006_HEIGHT, opStackGetValue(local+1).UInt);
  return 0;
}
...
char drawChar(){
          ...
          //Char into String
				  ...
				  et024006_PrintString(c, font, x, opStackGetValue(local+4).UInt, opStackGetValue(local+5).UInt, opStackGetValue(local+6).UInt);
				  return 0;
...
char isButtonPressed(){
  opStackPush((slot) (u4) qt60168_is_key_pressed(opStackGetValue(local+1).UInt)); 
  return 1;
}
