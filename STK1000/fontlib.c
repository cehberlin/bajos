...
//    }
  ...
  // falls FixedFont müssen die Zeichen innerhalb lcd_FontWidth zentriert werden, aber nur
  // wenn das Zeichen schmaler als lcd_FontWidth ist. Wir füllen dann auch gleich den rechten
...
//    }
    ...
    u2 bitsmask = 0xFF >> (8 - bitspixel);                // Bitmaske für Farbe
    u4 bits;                                             // enthält die aktuelle Bits des Zeichens
    u2 bitscount;                                         // aktuelle Anzahl an Bits in Bits
    u2 table[4];                                          // RLE Längen Tabelle
...
//      }
  ...
  // wir senden die Bitdaten der einzelnen Pixel jeweils Spaltenweise in Y Richtung
    ...
    u2 clipinvalid = 1;                                // Gültigkeit der Adresszeiger im PCF8833
    lcdCoord_t clipY = 0;                                  // letzte gesetzte Y-Adresse im PCF8833
    lcdCoord_t stopY = lcd_Cursor.Y + lcd_FontHeight -1; // unterste Pixelzeile des Zeichens
